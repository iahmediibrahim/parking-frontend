...
import { WsMessage, AdminUpdateMessage } from '@/types'
...
interface WebSocketState {
	...
	debugMode: boolean
	connectionHistory: Array<{ timestamp: Date; event: string; details?: any }>
	...
	addDebugMessage: (message: string, details?: any) => void
}
...
export const useWebSocketStore = create<WebSocketState>()(
	persist(
		(set, get) => ({
			...
			addDebugMessage: (message: string, details?: any) => {
				const { debugMode } = get()
				if (debugMode) {
					try {
				...
				try {
					...
					ws.onmessage = (event) => {
						...
						try {
							...
							if (
								(message as any).type === 'admin-update' ||
								(message as any).channel === 'admin'
							) {
								const adminUpdate = {
								...
								set((state) => ({
									adminUpdates: clampArray(
										[...state.adminUpdates, adminUpdate as AdminUpdateMessage],
										ADMIN_UPDATES_LIMIT,
									),
								}))
